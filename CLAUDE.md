...
- `src/data/suppliers.ts` — the feed bound to the ports the core defines, and no law of its
  own. `core/` owns what a still-possible set IS; each of those laws takes an already-chosen
  `RandbatsEntry`, and FINDING that entry means reading the feed, which core may not do. So
  the adapters sit here at the data edge and expose `DefenderVariantsFor` /
  `IncomingMovesFor` (both declared in `core/types.ts`, so an adapter and its consumer name
  the same type without importing each other). The assumption-driven half of the same pair is
  `core/assume.ts`'s own `openVariantsFor`, which needs no feed. The tell for where a function
  belongs: taking `RandbatsData` puts it here, taking a `RandbatsEntry` puts it in core.
...
| The matchup view's defensive half — the `Incoming:` group | ✅ | `data/suppliers.ts` (`randbatsIncomingMovesFor`), `section.ts` (`ownMovesSection`) | `section.test.ts`, `render.test.ts` |
...
| A disguised Zoroark surfaces as its own candidate, never a corrupted one | ✅ | `core/illusion.ts`, `data/suppliers.ts` (`illusionVariants`) | `illusion.test.ts`, `variants.test.ts`, `render.test.ts` |
