...
| Taking entry-hazard damage rules Heavy-Duty Boots out; switching in unharmed confirms it — Stealth Rock/G-Max Steelsurge confirm outright (nothing but Boots/Magic Guard dodges either), Spikes only once natural airborne-ness (Flying type, Levitate) is excluded too, since Spikes alone hits a grounded target only | ✅ | `core/deductions.ts` (`bootsRuledIn`), `battle/readState.ts` (`switchedIntoUnavoidableHazardUnharmed`, `switchedIntoSpikesUnharmed`) | `deductions.test.ts`, `resolve.test.ts`, `readState.test.ts` |
...
| A state change only stales a magnitude reading when it could actually move the calc's answer — a DEFENDER status change only counts for the handful of moves whose own power reads it (Hex, Venoshock, Wake-Up Slap, Smelling Salts), and an item change only counts when the item isn't a status-cure/HP-restore berry (never a damage number) | ✅ | `battle/readState.ts` (`stalesReading`, `DEFENDER_STATUS_DEPENDENT_MOVES`, `DAMAGE_IRRELEVANT_ITEMS`) | `readState.test.ts` |
...
| A log-derived reveal survives OUR OWN witness leaving the field for good — the fact it proves is about the FOE, not about whichever of ours is still standing | ✅ | `battle/readState.ts` (`witnessesAgainst`, `mostRecentCleanOrder`'s `them`-only invalidation), `section.ts` (`revealsAgainst`) | `readState.test.ts`, `section.test.ts` |
