...
| A magnitude reading is judged under the boosts the hit LANDED under, never the ones standing now — a move's own secondary drop lands between the two | ✅ | `battle/readState.ts` (`applyBoostLine`, `REPLAYABLE_BOOST_TAGS`), `core/itemreveal.ts` | `readState.test.ts`, `itemreveal.test.ts`, `section.test.ts` |
